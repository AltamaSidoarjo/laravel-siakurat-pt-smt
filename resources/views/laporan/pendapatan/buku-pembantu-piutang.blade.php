...
@section('title', 'Rincian Buku Pembantu Piutang')
...
@section('content')
    <div class="row mb-3 no-print">
        <div class="col">
            <div class="d-flex align-items-center gap-3 fs-3">
                <a href="{{ route('laporan.pendapatan.index') }}" class="text-dark" aria-label="Kembali ke laporan pendapatan">
                    <i class="bi bi-arrow-left"></i>
                </a>
                <span class="fw-bold">Rincian Buku Pembantu Piutang</span>
            </div>
        </div>
    </div>
    ...
    <div class="card border-muhammadiyah mb-2 report-shell">
        <div class="card-body d-flex flex-column gap-3">
            ...
            @include('laporan.pendapatan._piutang-filter', [
                'formAction' => route('laporan.pendapatan.buku-pembantu-piutang'),
                'resetAction' => route('laporan.pendapatan.buku-pembantu-piutang'),
                'exportAction' => route('laporan.pendapatan.buku-pembantu-piutang.export-csv'),
            ])
            ...
            <div class="aging-report" id="agingReport">
                <header class="report-header text-center mb-4">
                    ...
                    <h1>RINCIAN BUKU PEMBANTU PIUTANG</h1>
                ...
                @forelse ($cards as $card)
                    <section class="customer-section">
                        <h2 class="customer-title">
                            {{ strtoupper($card['nama_pelanggan']) }} | {{ $card['kode_pelanggan'] ?: '-' }} (IDR)
                        </h2>
                                    ...
                                    @foreach ($card['rows'] as $row)
                                        <tr>
                                            ...
                                            <td class="text-nowrap">
                                                <a href="{{ route('pendapatan.invoice.read', $row['faktur_id']) }}" class="reference-link">
                                                    {{ $row['nomor_referensi'] }}
                                                </a>
                                            </td>
                ...
                @empty
                    <div class="empty-report text-center text-muted py-5">
                        ...
                        <p class="mb-1 fw-semibold">Tidak ada piutang terbuka pada tanggal laporan.</p>
                        <p class="mb-0 no-print">Coba ubah tanggal laporan atau pilihan pelanggan.</p>
                    </div>
                @endforelse
                ...
                @if ($cards !== [])
                    ...
                    <div class="text-end fw-bold mt-2 report-balance">
                        Total Piutang: IDR {{ number_format($summary['saldo_piutang'], 2, ',', '.') }}
                    </div>
                @endif
            </div>
        </div>
