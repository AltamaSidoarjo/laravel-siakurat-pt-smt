...
@section('title', 'Buku Pembantu Piutang (Mutasi)')
    ...
    <div class="row mb-3">
        <div class="col">
            <div class="d-flex align-items-center gap-3 fs-3">
                ...
                <span class="fw-bold">Buku Pembantu Piutang (Mutasi)</span>
            </div>
        </div>
    </div>
    ...
    <div class="card border-muhammadiyah mb-2">
        <div class="card-body d-flex flex-column gap-3">
            ...
            <div class="card border-light shadow-sm">
                <div class="card-body">
                    <form method="get" action="{{ route('laporan.pendapatan.buku-pembantu-piutang-mutasi') }}" id="filterForm">
                        <div class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-6 d-flex align-items-end justify-content-end gap-2 flex-wrap">
                                <a href="{{ route('laporan.pendapatan.buku-pembantu-piutang-mutasi') }}" class="btn btn-light">Reset</a>
                                <button type="submit" class="btn btn-primary"><i class="bi bi-funnel me-1"></i>Tampilkan</button>
                                <button type="submit" class="btn btn-outline-success" formaction="{{ route('laporan.pendapatan.buku-pembantu-piutang-mutasi.export-csv') }}" @disabled(! $hasSelection)>
            ...
            @else
                ...
                <div class="row row-cols-1 row-cols-md-2 row-cols-xl-4 g-3">
                    ...
                    <div class="col"><div class="card h-100 border-light shadow-sm"><div class="card-body"><div class="text-muted small">Penambahan Piutang</div><div class="fw-bold fs-5 text-primary">Rp {{ number_format($summary['total_debit'], 2, ',', '.') }}</div></div></div></div>
                    <div class="col"><div class="card h-100 border-light shadow-sm"><div class="card-body"><div class="text-muted small">Penerimaan</div><div class="fw-bold fs-5 text-success">Rp {{ number_format($summary['total_kredit'], 2, ',', '.') }}</div></div></div></div>
                ...
                <div class="card border-light shadow-sm">
                    <div class="card-body">
                        ...
                        <input type="search" id="transactionSearch" class="form-control" placeholder="Cari nomor faktur, penerimaan, pelanggan, akun, atau keterangan...">
                        <div class="text-muted small mt-2" id="searchResultInfo"></div>
                    </div>
                </div>

                <div id="pelangganCards" class="d-flex flex-column gap-3">
                    @forelse ($cards as $card)
                        <div class="card border-light shadow-sm pelanggan-card" data-pelanggan="{{ strtolower($card['kode_pelanggan'].' '.$card['nama_pelanggan']) }}">
                            <div class="card-header bg-white d-flex flex-wrap align-items-center justify-content-between gap-2">
                                <div>
                                    <div class="fw-bold text-primary">[{{ $card['kode_pelanggan'] ?: '-' }}] {{ $card['nama_pelanggan'] }}</div>
                                    <div class="text-muted small">Akun piutang: {{ $card['akun'] ? implode(', ', $card['akun']) : '-' }}</div>
                                </div>
                                @if ($card['saldo_akhir'] > 0)
                                    <span class="badge bg-danger">Saldo Piutang: Rp {{ number_format($card['saldo_akhir'], 2, ',', '.') }}</span>
                    ...
                    @empty
                        <div class="card border-light shadow-sm"><div class="card-body text-center text-muted py-5"><i class="bi bi-inbox fs-1 d-block mb-3"></i><p class="mb-1 fw-semibold">Tidak ada data piutang sesuai filter.</p><p class="mb-0">Coba ubah periode, pelanggan, atau status saldo akhir.</p></div></div>
                    @endforelse
                </div>
            @endif
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            searchInput?.addEventListener('input', function () {
                ...
                let visibleRows = 0;
                let totalRows = 0;
                document.querySelectorAll('.pelanggan-card').forEach(function (card) {
                    const pelangganMatched = keyword !== '' && (card.dataset.pelanggan || '').includes(keyword);
                    let cardHasMatch = keyword === '' || pelangganMatched;
                    card.querySelectorAll('tbody .ledger-row').forEach(function (row) {
                        totalRows++;
                        const matched = keyword === '' || pelangganMatched || row.textContent.toLowerCase().includes(keyword);
