@php
    ...
    $rincianPrint = $penerimaanPendapatan->rincian->filter(fn ($item) => (float) $item->nominal_bayar > 0)->values();
    $totalGrand = $rincianPrint->sum(fn ($item) => (float) ($item->fakturPenjualan?->grandtotal ?? 0));
...
<head>
    ...
    <title>PPD : {{ $penerimaanPendapatan->nomer }}</title>
...
<body>
    ...
    <div class="wrapper">
        ...
        <table class="table-print">
            ...
            <tbody>
                <tr>
                    <td class="wrap-text"><span class="label-strong">Dokumen:</span> Penerimaan Pendapatan</td>
                    <td class="wrap-text"><span class="label-strong">Nomor:</span> {{ $penerimaanPendapatan->nomer }}</td>
                    <td class="wrap-text"><span class="label-strong">Tanggal:</span> {{ optional($penerimaanPendapatan->tanggal)->format('d/m/Y') }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table-print">
            ...
            <tbody>
                <tr>
                    <td class="wrap-text">
                        <span class="label-strong">Penjamin/Pelanggan:</span><br>
                        {{ $penerimaanPendapatan->pelanggan?->nama_pelanggan }}
                    </td>
                    <td class="wrap-text">
                        <span class="label-strong">Bank:</span><br>
                        {{ $penerimaanPendapatan->akunBank?->kode }} - {{ $penerimaanPendapatan->akunBank?->nama }}
                    </td>
                </tr>
                <tr>
                    <td class="wrap-text">
                        <span class="label-strong">Akun Piutang:</span><br>
        ...
        <table class="table-print mt-2">
            ...
            <tbody>
                @forelse ($rincianPrint as $item)
                    ...
                    <tr>
                        <td class="wrap-text">
                            <div><span class="label-strong">Nomor:</span> {{ $item->fakturPenjualan?->nomor_faktur }}</div>
                            <div><span class="label-strong">Tanggal:</span> {{ optional($item->fakturPenjualan?->tanggal_faktur)->format('d M Y') }}</div>
                            <div><span class="label-strong">Pasien:</span> {{ $item->fakturPenjualan?->nama_pasien }}</div>
                            <div><span class="label-strong">No. RM:</span> {{ $item->fakturPenjualan?->nomer_rekam_medis }}</div>
                        </td>
