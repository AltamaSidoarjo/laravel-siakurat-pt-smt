@php
    ...
    $nominalTotal = (int) round((float) $kasbankPembayaran->total);
...
<head>
    ...
    <title>BK : {{ $kasbankPembayaran->nomer }}</title>
...
<body>
    ...
    <div class="wrapper">
        ...
        <table class="table-print">
            ...
            <tbody>
                <tr>
                    <td class="wrap-text"><b>Bukti Keluar:</b> {{ $kasbankPembayaran->coa?->kode }} - {{ $kasbankPembayaran->coa?->nama }}</td>
                    <td class="wrap-text"><b>Nomer:</b> {{ $kasbankPembayaran->nomer }}</td>
                    <td class="wrap-text"><b>Tanggal:</b> {{ optional($kasbankPembayaran->tanggal)->format('d/m/Y') }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table-print mt-2">
            ...
            <tbody>
                @forelse ($kasbankPembayaran->rincian as $item)
                    <tr>
                        <td class="wrap-text">{{ $item->coa?->kode }} - {{ $item->coa?->nama }}</td>
                        <td class="wrap-text">{{ $item->catatan }}</td>
                        <td class="num">{{ $formatRupiah($item->nominal) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="3" class="wrap-text text-center">Belum ada rincian ditambahkan</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="2" class="num" style="font-weight:700;">Jumlah</td>
                    <td class="num" style="font-weight:700;">{{ $formatRupiah($kasbankPembayaran->total) }}</td>
                </tr>
            </tfoot>
        </table>
        ...
        <table class="table table-bordered border-dark mt-3 text-center">
            <tr>
                ...
                <td style="width:34%;">Dibukukan oleh</td>
            </tr>
            <tr>
