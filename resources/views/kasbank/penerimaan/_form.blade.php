@php
    $isEdit = isset($kasbankPenerimaan);
    $rincian = old('rincian', $isEdit ? $kasbankPenerimaan->rincian->map(fn ($item) => [
        'coa_id' => $item->coa_id,
    ...
    <div class="card border-light shadow-sm">
        ...
        <div class="card-body">
            <div class="row align-items-center mb-2">
                ...
                <div class="col">
                    <input type="text" name="nomer" id="input_nomor" class="form-control" value="{{ old('nomer', $kasbankPenerimaan->nomer ?? '') }}" required>
                </div>
            </div>

            <div class="row align-items-center mb-2">
                <label for="select_coa" class="col-12 col-sm-2 col-form-label fw-bold">
                    Diterima ke<span class="text-danger">*</span>
                </label>
                <div class="col">
                    <select name="coa_id" id="select_coa" class="form-select select2-coa" required>
                        <option value="">Pilih Akun</option>
                        @foreach ($coaOptions as $coa)
                            <option value="{{ $coa->id }}" @selected((string) old('coa_id', $kasbankPenerimaan->coa_id ?? '') === (string) $coa->id)>
                                {{ $coa->kode }} - {{ $coa->nama }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row align-items-center mb-2">
                ...
                <div class="col">
                    <input type="date" name="tanggal" id="input_tanggal" class="form-control" value="{{ old('tanggal', isset($kasbankPenerimaan) ? optional($kasbankPenerimaan->tanggal)->format('Y-m-d') : '') }}" required>
                </div>
            </div>
            ...
            <div class="row align-items-center mb-2">
                ...
                <div class="col">
                    <textarea name="keterangan" id="input_keterangan" class="form-control" rows="3">{{ old('keterangan', $kasbankPenerimaan->keterangan ?? '') }}</textarea>
                </div>
            </div>
        </div>
    </div>
                ...
                <table id="table_data_detail" class="table table-sm table-bordered">
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>
                                <input type="text" name="total" id="input_total" class="form-control text-end" value="{{ number_format((float) old('total', $kasbankPenerimaan->total ?? 0), 0, ',', '.') }}" readonly>
                            </td>
                            <td></td>
                        </tr>
                    </tfoot>
        ...
        <div class="card-body">
            <div class="d-flex {{ $isEdit ? 'justify-content-between' : 'justify-content-end' }} gap-3">
                ...
                <div class="d-flex gap-3">
                    <a href="{{ route('kasbank.penerimaan.index') }}" class="btn btn-light fw-bold">
                    ...
                    @else
                        <a class="btn btn-primary fw-bold" target="_blank" href="{{ route('kasbank.penerimaan.print', $kasbankPenerimaan) }}">
